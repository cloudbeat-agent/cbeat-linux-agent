...
echo "--------------------------------"
echo " Welcome to cBeat Agent Installer"
echo "--------------------------------"
echo " "

# Are we running as root
if [ $(id -u) != "0" ]; then
	echo "cBeat Agent installer needs to be run with root priviliges"
...
wget -N --no-check-certificate -O /opt/cbeat/agent.sh https://raw.githubusercontent.com/cloudbeat-agent/cbeat-linux-agent/main/agent.sh >> $LOG 2>&1
wget -N --no-check-certificate -O /opt/cbeat/uninstall.sh https://raw.githubusercontent.com/cloudbeat-agent/cbeat-linux-agent/main/uninstall.sh >> $LOG 2>&1
...
useradd cbeatagent -r -d /opt/cbeat -s /bin/false >> $LOG 2>&1
groupadd cbeatagent >> $LOG 2>&1

# Disable cagefs for cbeat
if [ -f /usr/sbin/cagefsctl ]; then
	/usr/sbin/cagefsctl --disable cbeatagent >> $LOG 2>&1
fi

# Modify user permissions
chown -R cbeatagent:cbeatagent /opt/cbeat && chmod -R 700 /opt/cbeat >> $LOG 2>&1

# Configure cron
crontab -u cbeatagent -l 2>/dev/null | { cat; echo "* * * * * bash /opt/cbeat/agent.sh > /opt/cbeat/cron.log 2>&1"; } | crontab -u cbeatagent -
